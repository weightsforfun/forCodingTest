...
def getSumOfStat(teamMember):  ## 조합으로 만든 팀 구성원 넣어주면 그 팀의 시너지값을 return함
    ...
    for i in range(0,len(teamMember)):
        for j in range(i+1,len(teamMember)):  
            SumofStat+=(ability[teamMember[i]][teamMember[j]]+ability[teamMember[j]][teamMember[i]])
    return SumofStat


diff=1001  # 나올수있는 차의 최대값+1
for i in range(len(comb)//2): ## 만들어진 모든 조합중 반만 해도 상태딤이 그 절반이니까 반만돌린다 i우리팀 -1-i반대편부터 상대팀으로 차이구해줌
    diff=min(diff,abs(getSumOfStat(comb[i])-getSumOfStat(comb[-i-1])))
